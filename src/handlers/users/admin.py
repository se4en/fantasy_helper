...
@dp.callback_query_handler(
    admin_callback.filter(
        tool_name=["update_players", "update_coeffs", "update_stats"],
        league_name="None",
    )
)
...
@dp.callback_query_handler(
    admin_callback.filter(
        tool_name=["update_players", "update_coeffs", "update_stats", "update_all"]
    )
)
async def admin_update_resource(call: CallbackQuery, callback_data: dict):
    ...
    if callback_data["tool_name"] == "update_players":
        ...
        await update_all()
    await call.message.answer(
        text="Доступные инструменты:", reply_markup=create_admin_keyboard()
    )
...
async def admin_manage_source_menu(call: CallbackQuery, callback_data: dict):
    await call.answer(cache_time=10)
    await call.message.answer(
        text="Выберите чемпионат:", reply_markup=create_sources_keyboard("add")
    )
...
async def admin_manage_source_menu(call: CallbackQuery, callback_data: dict):
    await call.answer(cache_time=10)
    await call.message.answer(
        text="Выберите чемпионат:", reply_markup=create_sources_keyboard("delete")
    )


@dp.callback_query_handler(
    admin_callback.filter(
        tool_name="back_to_list",
    )
)
