...
class XBet:
    ...
    async def __update_match(
        self, league_name: str, match_info: dict, cur_round: bool = False
    ) -> bool:
        ...
        try:
            ...
            if (
                "голы" in home_team
                or "специальное" in home_team
                or "Хозяева" in home_team
            ):
                return False
            ...
            total_1_more_1_5 = float(
                total_1_all.find("div", containing="1.5 Б", first=True)
                .find("i", first=True)
                .text
            )
            total_1_less_0_5 = float(
                total_1_all.find("div", containing="0.5 М", first=True)
                .find("i", first=True)
                .text
            )

            total_2_all = game_html.find(containing="Индивидуальный тотал 2-го")[4]
            total_2_more_1_5 = float(
                total_2_all.find("div", containing="1.5 Б", first=True)
                .find("i", first=True)
                .text
            )
            total_2_less_0_5 = float(
                total_2_all.find("div", containing="0.5 М", first=True)
                .find("i", first=True)
                .text
            )
    ...
    async def update_league(self, league_name: str) -> bool:
        ...
        try:
            ...
            while updated_matches < cur_round_matches:
                if await self.__update_match(
                    league_name, all_matches[checked_matches], True
                ):
                    updated_matches += 1
                checked_matches += 1
            # update next round
            while checked_matches < len(all_matches):
                await self.__update_match(
                    league_name, all_matches[checked_matches], False
                )
                checked_matches += 1
    ...
    def update_all(self) -> bool:
        return all(
            list(map(lambda x: asyncio.run(self.update_league(x)), self.leagues))
        )
