...
from keyboards.inline.sources_buttons import (
    create_sources_keyboard,
    create_sources_back_keyboard,
    create_sources_league_keyboard,
    create_delete_source_keyboard,
)
from fantasy_helper.manager_loader import sources_manager


@dp.callback_query_handler(
    sources_callback.filter(
        league_name="cancel",
    )
)
...
async def add_new_source(call: CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    await call.message.answer(
        "Введите название (коротко):", parse_mode=ParseMode.MARKDOWN
    )
...
async def set_new_description(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        sources_manager.add_source(
            data["name"], data["league"], data["url"], message.text
        )
    await state.finish()
    await message.answer(
        sources_manager.get_sources_repr(data["league"]),
        reply_markup=create_sources_league_keyboard(sources_callback, data["league"]),
        parse_mode=ParseMode.MARKDOWN,
...
async def delete_source(call: CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    await call.message.answer(
        ...
        reply_markup=create_delete_source_keyboard(
            sources_callback, callback_data["league_name"]
        ),
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def delete_source(call: CallbackQuery, state: FSMContext, callback_data: dict):
    ...
    await call.message.answer(
        sources_manager.get_sources_repr(league_name),
...
async def sources_list(call: CallbackQuery, callback_data: dict):
    await call.answer(cache_time=10)
    await call.message.answer(
        sources_manager.get_sources_repr(callback_data["league_name"]),
        reply_markup=create_sources_league_keyboard(
            sources_callback, callback_data["league_name"]
        ),
        parse_mode=ParseMode.MARKDOWN,
        disable_web_page_preview=True,
