...
from keyboards.inline.coeff_buttons import create_coeff_leagues_keyboard, create_coeff_back_keyboard


@dp.callback_query_handler(coeffs_callback.filter(league_name="cancel", ))
...
@dp.callback_query_handler(coeffs_callback.filter(league_name="back_to_list", ))
...
async def get_coeffs(call: CallbackQuery, callback_data: dict):
    await call.answer(cache_time=10)
    await call.message.answer(text=coeff_manager.get_coeffs(callback_data["league_name"],
                                                            cur_round=(callback_data["round"] == "cur")),
