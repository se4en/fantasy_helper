...
async def back_to_coefs(call: CallbackQuery, callback_data: dict):
    await call.answer(cache_time=10)
    await call.message.answer("Доступные чемпионаты:", reply_markup=create_country_keyboard(players, players_callback), parse_mode=ParseMode.MARKDOWN)
...
async def get_coefs(call: CallbackQuery, callback_data: dict):
    ...
    for players_legue in players:
        if players_legue.get_name()==legue_name:
            await call.message.answer(text=players_legue.get_popular(), reply_markup=create_country_back_keyboard(players_callback), parse_mode=ParseMode.MARKDOWN)